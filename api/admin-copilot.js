...
const CUSTOM_LABEL_FIELDS = Object.freeze({ packages: new Set(['category']), destinations: new Set(['category']) });
const CUSTOM_SENTINELS = new Set(['other', 'custom', '__other__', '__custom__']);
...
function sanitizeCustomLabels(entity, value, patch) {
  const allowed = CUSTOM_LABEL_FIELDS[entity];
  if (!allowed || !isPlainObject(value)) return {};
  const output = {};
  for (const [field, raw] of Object.entries(value)) {
    if (!allowed.has(field) || typeof raw !== 'string') continue;
    const label = raw.trim().slice(0, 120);
    if (!label || CUSTOM_SENTINELS.has(label.toLowerCase()) || String(patch?.[field] || '').trim() !== label) continue;
    output[field] = label;
  }
  return output;
}
...
function sanitizeEditorPrefill(value, trustedImageUrls = [], verifiedSources = [], pageContext = null) {
  ...
  const candidate = { ...raw, patch: normalizeEditorPrefillPatch(raw.entity, raw.patch) };
  ...
  const rawLabels = raw.customLabels || (typeof raw.customLabel === 'string' ? { category: raw.customLabel } : null);
  const customLabels = sanitizeCustomLabels(draft.entity, rawLabels, draft.patch);
  return Object.keys(customLabels).length ? { ...draft, customLabels } : draft;
}
